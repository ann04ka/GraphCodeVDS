...
В данной работе предложен подход к обнаружению уязвимостей в исходном коде, основанный на глубоком анализе результатов масштабного исследования "Vulnerability Detection with Code Language Models: How Far Are We?" и его датасета PrimeVul. 
Несмотря на то что предложенное решение не претендует на статус state-of-the-art, оно демонстрирует улучшенные метрики обнаружения уязвимостей в классе компактных моделей (до 250М параметров) по сравнению с результатами, представленными в исходной статье. Ключевой идеей подхода является комбинация техник контрастивного обучения с добавлением более глубокой классификационной головы, позволяющей модели выучить более выразительные представления для различения уязвимого и безопасного кода. Эксперименты проведены исключительно на датасете PrimeVul, который благодаря ручной валидации и отсутствию утечек между обучающей и тестовой выборками обеспечивает надежную оценку обобщающей способности моделей. 
Полученные результаты будут основой для дальнейшего исследования и разработки решений обнаружения уязвимостей в коде при ограничениях на размер модели.


Предварительный анализ пространства эмбеддингов демонстрирует способность предобученных моделей выявлять закономерности в исходном коде, что служит основанием для дальнейшего исследования.
